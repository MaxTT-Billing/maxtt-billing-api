...
app.post('/api/admin/franchisees/install', requireSA, async (_req, res) => {
  ...
  try {
    ...
    await client.query(`CREATE TABLE IF NOT EXISTS public.franchisees ( id BIGSERIAL PRIMARY KEY );`)
...
function istMonthBoundsUTC(d=new Date()){
  ...
  const istMs = utc.getTime() + istOffsetMin*60*1000
  ...
  return { startUTC: new Date(startIST.getTime()-istOffsetMin*60*1000),
           endUTC: new Date(endIST.getTime()-istOffsetMin*60*1000) }
}
async function computeNextNumbers(client, cols, franchiseeId){
  ...
  const today=new Date(), mm=String(today.getUTCMonth()+1).padStart(2,'0'), yy=String(today.getUTCFullYear()).slice(-2), mmyy=`${mm}${yy}`

  let nextMonthly=1
  if (invNoCol){
    const r=await client.query(`
      SELECT ${qid(invNoCol)} AS inv FROM public.invoices
      WHERE ${qid(invNoCol)} ILIKE $1 ORDER BY 1 DESC LIMIT 300
    `,[`${franchiseeId}/%/${mmyy}%`])
    for(const row of r.rows){
      const m=String(row.inv||'').match(/^[A-Z-0-9-]+\/(\d{4})\/(\d{4})$/i)
      if(m){ const seq=Number(m[1]||'0'); if (Number.isFinite(seq) && seq>=nextMonthly) nextMonthly=seq+1 }
    }
  }
  if (nextMonthly===1 && has(cols,'invoice_seq')){
    ...
    `,[franchiseeId,startUTC.toISOString(),endUTC.toISOString()])
  ...
  if (custCodeCol){
    const r=await client.query(`
      SELECT ${qid(custCodeCol)} AS cc FROM public.invoices
      WHERE ${qid(custCodeCol)} ILIKE $1 ORDER BY 1 DESC LIMIT 200
    `,[`${franchiseeId}-%`])
    for(const row of r.rows){
      const m=String(row.cc||'').match(/^[A-Z-0-9-]+-(\d{4,})$/i)
      if(m){ const seq=Number(m[1]||'0'); if(Number.isFinite(seq) && seq>=nextCust) nextCust=seq+1 }
    }
  }
...
function recommendPerTyreMl(widthMm=195, rimIn=15){
  const w=Number(widthMm)||195, r=Number(rimIn)||15
  let base=260
  if (w<=165) base=220
...
app.post('/api/invoices/full', async (req,res)=>{
  ...
  try{
    ...
    const unitPrice =
      (unitPriceCol ? (asNum(payload[unitPriceCol]) ?? asNum(payload.mrp_per_ml) ?? asNum(payload.price_per_ml))
                    : (asNum(payload.mrp_per_ml) ?? asNum(payload.price_per_ml))) ?? fallbackUnit
    ...
    const accepted=[
      ...
      'invoice_seq','invoice_number_norm',
...
app.post('/api/admin/franchisees/resubmit/:id', requireAdmin, async (req,res)=>{
  ...
  try{
    ...
    for (const k of editable){ if (b[k]!==undefined){ sets.push(`${qid(k)}=$${i++}`); params.push(b[k]) } }
