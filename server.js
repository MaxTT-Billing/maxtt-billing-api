...
// Franchisee login + profile (SET THESE IN RENDER → API)
const FRANCHISEE_ID       = process.env.FRANCHISEE_ID || "";       // e.g. MAXTT-DEL-001
const FRANCHISEE_PASSWORD = process.env.FRANCHISEE_PASSWORD || ""; // strong password
...
// ======= in-memory token store (franchisee login) =======
...
// Profile → now includes franchisee_id (so invoice header can show it)
...
// Create invoice (requires x-api-key + Bearer token)
app.post("/api/invoices", requireApiKey, requireAuth, async (req, res) => {
  try {
    ...
    // IMPORTANT: we stamp the invoice with the franchisee_id from env
    const q = `
      INSERT INTO invoices
        (customer_name, mobile_number, vehicle_number, odometer, tread_depth_mm, installer_name, vehicle_type,
         ...
         fitment_locations, customer_gstin, customer_address, franchisee_id)
      VALUES
    ...
    const vals = [
      ...
      tyre_width_mm, aspect_ratio, rim_diameter_in, dosage_ml, price_per_ml, gst_rate,
      total_before_gst, gst_amount, total_with_gst, gps_lat, gps_lng, customer_code, tyre_count,
      fitment_locations || null, customer_gstin || null, customer_address || null, FRANCHISEE_ID || null
    ];
