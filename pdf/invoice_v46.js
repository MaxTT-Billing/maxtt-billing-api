// pdf/invoice_v46.js — exact v46 layout clone (no logo/watermark)
// Uses PDFKit base fonts; coordinates tuned to #46 grid.
// Zone-2 includes HSN Code + Customer ID (per your rule).
...
export async function createV46Pdf(stream, inv, fr) {
  return new Promise((resolve,reject)=>{
    ...
    // === Header (matches #46 two-column box layout) ===
    // Left (franchisee)
    doc.font('Helvetica-Bold').fontSize(12).text(safe(fr?.legal_name,'Franchisee'), 36, 36, { width: 320 })
    doc.font('Helvetica').fontSize(9)
      .text(safe([fr?.address1, fr?.address2].filter(Boolean).join(', '), 'Address not set'), 36, 54, { width: 320 })
      .text(`Franchisee ID: ${safe(inv.franchisee_id || inv.franchisee_code)}`, 36, 68, { width: 320 })
    const gstin = safe(fr?.gstin,'')
    if (gstin !== '—') doc.text(`GSTIN: ${gstin}`, 36, 82, { width: 320 })
    ...
    doc.text(`Customer ID: ${safe(inv.customer_code || inv.invoice_number_norm)}`, 44, z2Y+86, { width: 250 })
    ...
    for (const [pos, val] of rows){
      doc.font('Helvetica').fontSize(10).text(pos, 44, ry, { width: 240 })
    ...
    const Lx = 44, Rx = 340
    const V = (y, k, v) => { doc.font('Helvetica').fontSize(10).text(k, Lx, y); doc.text(v, Rx, y, { width: 200, align:'right' }) }
    ...
    const TV = (t,y)=> doc.font('Helvetica').fontSize(10).text(t, 340, y, { width:200, align:'right' })
    ...
    doc.font('Helvetica').fontSize(9)
      .text('1. I hereby acknowledge that the MaxTT Tyre Sealant installation has been completed on my vehicle to my satisfaction, as per my earlier consent to proceed.', 36, decY+14, { width:520 })
      .text('2. I have read, understood, and accepted the Terms & Conditions stated herein.', 36, decY+30, { width:520 })
      .text('3. I acknowledge that the total amount shown is correct and payable to the franchisee/installer of Treadstone Solutions.', 36, decY+46, { width:520 })
    doc.font('Helvetica-Bold').fontSize(9).text('Terms & Conditions', 36, decY+66, { underline:true })
    doc.font('Helvetica').fontSize(9)
      .text('1. The MaxTT Tyre Sealant is a preventive safety solution designed to reduce tyre-related risks and virtually eliminate punctures and blowouts.', 36, decY+80, { width:520 })
      .text('2. Effectiveness is assured only when the vehicle is operated within the speed limits prescribed by competent traffic/transport authorities (RTO/Transport Department) in India.', 36, decY+96, { width:520 })
      .text('3. Jurisdiction: Gurgaon.', 36, decY+112, { width:520 })

    const sigY = decY + 132
    doc.rect(36, sigY, 240, 58).stroke(); doc.font('Helvetica').fontSize(9).text('Installer Signature & Stamp', 44, sigY+42)
    doc.rect(320, sigY, 240, 58).stroke(); doc.text('Customer Accepted & Confirmed', 328, sigY+42)
